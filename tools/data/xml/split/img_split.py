...
def load_xml(img_dir, ann_dir=None, nproc=10):
    """Load XML dataset.
    ...
    print('Starting loading XML dataset information.')
    start_time = time.time()
    _load_func = partial(_load_xml_single, img_dir=img_dir, ann_dir=ann_dir)
    if nproc > 1:
        pool = Pool(nproc)
    ...
    end_time = time.time()
    print(f'Finishing loading XML, get {len(contents)} iamges,',
          f'using {end_time - start_time:.3f}s.')

    return contents


def _load_xml_single(imgfile, img_dir, ann_dir):
    """Load XML's single image.
    ...
    if xmlfile is None:
        ...
        if None != filecontent:
            ...
            for obj in dom.getElementsByTagName('object'):
                ...
                cx = obj.getElementsByTagName('cx')[0].firstChild.data
                cy = obj.getElementsByTagName('cy')[0].firstChild.data
                w = obj.getElementsByTagName('w')[0].firstChild.data
                h = obj.getElementsByTagName('h')[0].firstChild.data
                angle = obj.getElementsByTagName('angle')[0].firstChild.data
                obb = np.array([[cx, cy, w, h, angle,0]], dtype=np.float32)
                poly = obb2poly_np(obb, version='le90')

                bboxes.append(poly[0,:8])
                labels.append(label)
                diffs.append(0)
...
def main():
    ...
    for img_dir, ann_dir in zip(args.img_dirs, args.ann_dirs):
        _infos = load_xml(img_dir=img_dir, ann_dir=ann_dir, nproc=args.nproc)
