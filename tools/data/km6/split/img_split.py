...
def load_fair1m(img_dir, ann_dir=None, nproc=10):
    """Load FAIR1M dataset.
    ...
    _load_func = partial(_load_km6_single, img_dir=img_dir, ann_dir=ann_dir)
    if nproc > 1:
        pool = Pool(nproc)
    ...
    end_time = time.time()
    print(f'Finishing loading FAIR1M, get {len(contents)} iamges,',
          f'using {end_time - start_time:.3f}s.')

    return contents


def _load_km6_single(imgfile, img_dir, ann_dir):
    """Load FAIR1M's single image.
    ...
    if txtfile is None:
        ...
        with open(txtfile, 'r') as file:
            for line in file:
                line = line.strip('\n')
                contents = line.split()
                bboxes.append([float(i) for i in contents[1:8]])
...
def main():
    ...
    for img_dir, ann_dir in zip(args.img_dirs, args.ann_dirs):
        _infos = load_fair1m(
            img_dir=img_dir, ann_dir=ann_dir, nproc=args.nproc)
