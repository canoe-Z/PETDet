...
class LFFDecoupleHeadRoIHead(BaseModule, metaclass=ABCMeta):
    ...
    def _bbox_forward(self, x, y, rois):
        ...
        down_p2 = self.p2_conv(p2_feat)
        down_upsample_p2 = F.interpolate(
            down_p2, y.shape[2:], **self.upsample_cfg)
        cat_feat1 = torch.cat((down_upsample_p2, y), dim=1)
        down_cat_feat1 = self.fusion_conv(cat_feat1)
        cat_feat1 = self.up_conv(down_cat_feat1)
        att_feat1 = self.att_module_1(cat_feat1)
        #att_feat1 = att_feat1+p2_feat

        y_downsample = self.maxpool(y_out)
        ...
        cat_feat2 = torch.cat((p2_feat, y_downsample), dim=1)
        down_feat = self.down_conv(cat_feat2)
        att_feat2 = self.att_module(down_feat)
        ...
        lowlevel_feat = p2_feat + att_feat1 + att_feat2

        bbox_feats = self.bbox_roi_extractor(
            x[self.start_level:self.start_level + self.bbox_roi_extractor.num_inputs], lowlevel_feat, rois)
