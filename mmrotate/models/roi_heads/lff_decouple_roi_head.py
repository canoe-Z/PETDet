...
class LFFDecoupleHeadRoIHead(BaseModule, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        self.y_conv = ResLayer(block=BasicBlock, inplanes=64,
                               planes=256, num_blocks=3, stride=2,
                               init_cfg=[
    ...
    def _bbox_forward(self, x, y, rois):
        ...
        z = self.y_conv(y)
        # print(z)
        # bbox_cls_feats = self.cls_bbox_roi_extractor(
        #     [z], rois)
        # bbox_reg_feats = self.reg_bbox_roi_extractor(
        #     x[:self.reg_bbox_roi_extractor.num_inputs], rois)
        bbox_feats = self.bbox_roi_extractor(
            x[:self.bbox_roi_extractor.num_inputs], rois, backbone_feat=z)
