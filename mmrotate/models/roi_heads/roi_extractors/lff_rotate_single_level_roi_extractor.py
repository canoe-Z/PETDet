...
class DecoupleRotatedSingleRoIExtractor(BaseRoIExtractor):
    ...
    def forward(self, feats, rois, roi_scale_factor=None, backbone_feat=None):
        ...
        # Low Feature Fusion Module
        if backbone_feat is not None:
            for feat in feats:
                print(backbone_feat.size)
                print(feat.size)
                assert(1==2)
        reg_feats=feats
        cls_feats=feats
        ...
        for i in range(num_levels):
            ...
            if inds.numel() > 0:
                ...
                elif self.cls_feat == 'current':
                    roi_feats_f = roi_feats_c
