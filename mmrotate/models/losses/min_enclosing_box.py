...
def gather_lines_points(corners: torch.Tensor):
    ...
    """
    dim = corners.dim()
    idx_lines = torch.LongTensor(LINES).to(
        corners.device).unsqueeze(-1)      # (24, 2, 1)
    idx_points = torch.LongTensor(POINTS).to(
        corners.device).unsqueeze(-1)    # (24, 6, 1)
    # (24, 2, 2)
    ...
    if dim > 2:
        ...
        idx_points = idx_points.repeat(
            *corners.size()[:-2], 1, 1, 1)           # (..., 24, 2, 2)
        idx_lines = idx_lines.repeat(
            *corners.size()[:-2], 1, 1, 1)             # (..., 24, 6, 2)
    # (..., 24, 8, 2)
...
def point_line_distance_range(lines: torch.Tensor, points: torch.Tensor):
    ...
    # or, all points are on the same side
...
def point_line_projection_range(lines: torch.Tensor, points: torch.Tensor):
    ...
    # (..., 24, 2)
    vec = torch.cat(
        [torch.ones_like(k, dtype=k.dtype, device=k.device), k], dim=-1)
    vec = vec.unsqueeze(-2)             # (..., 24, 1, 2)
    # (..., 24, 8), consider all 8 points
...
def smallest_bounding_box(corners: torch.Tensor, verbose=False):
    ...
    fake = torch.ones_like(zero_mask, dtype=corners.dtype,
                           device=corners.device) * 1e8 * zero_mask
