...
from mmrotate.ops import convex_sort
from mmdet.models.builder import LOSSES
from mmdet.models.losses.utils import weighted_loss
from ...core.bbox.transforms import obb2poly


def get_bbox_areas(rbboxes):
    dim = rbboxes.size(-1)
    if dim == 4:
        wh = rbboxes[..., 2:] - rbboxes[..., :2]
        areas = wh[..., 0] * wh[..., 1]
    elif dim == 5:
        areas = rbboxes[..., 2] * rbboxes[..., 3]
    elif dim == 8:
        pts = rbboxes.view(*rbboxes.size()[:-1], 4, 2)
...
def convex_areas(pts, masks):
    ...
    # print(polys.shape)
    # print(polys[:, 0:-1, 0].shape)
    # print(polys[:, 1:, 1].shape)
    # print(polys[10,:,:])
    # assert(1==2)
...
def poly_intersection(pts1, pts2, areas1=None, areas2=None, eps=1e-6):
    ...
    with torch.no_grad():
        ...
        mask_inside1 = torch.abs(
            sum_areas1 - areas2[..., None]) < 1e-3 * areas2[..., None]

        triangle_areas2 = 0.5 * torch.abs(
            (x1 - x3) * (y2 - y3) - (x2 - x3) * (y1 - y3))
        sum_areas2 = triangle_areas2.sum(dim=-2)
        mask_inside2 = torch.abs(
            sum_areas2 - areas1[..., None]) < 1e-3 * areas1[..., None]
...
def poly_iou_loss(pred, target, linear=False, eps=1e-6):
    ...
    pred, target = obb2poly(pred), obb2poly(target)
